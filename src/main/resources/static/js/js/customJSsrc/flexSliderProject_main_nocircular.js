...
window.addEventListener('load', function() {
	...
	setTimeout(function() {
		...
		document.getElementById('flexSlider_preloader').style.animation = "preloaderRemoval 1s 1";
	...
	function buildFlexSlider(givenDataSet) {
		...
		var extractedRawCategories = Array.prototype.slice.call(givenDataSet).map(function(eachElement) {
			return String(eachElement.category);
		}).unique(); // e.g. Array ["사람", "음식", "동물"] is given.
		for (var categoriesCounter = 0; categoriesCounter < extractedRawCategories.length; categoriesCounter++) {
			...
			var itemSetofThisCategory = givenDataSet.filter(function(eachItem) {
				return eachItem.category.includes(extractedRawCategories[categoriesCounter]);
			});
			itemSetofThisCategory.sort(function (former, latter) { return former.id - latter.id; });
			
			var arrayForItemsInThisCategory = [];
			for (var itemCounter = 0; itemCounter < itemSetofThisCategory.length; itemCounter++) {
				/* Rendering the new item */
				var newItemContainerDarkWrapper = document.createElement('div');
				
				var headlineText = document.createTextNode(String(itemSetofThisCategory[itemCounter].name));
				var headlineTextNode = document.createElement('h2');
				...
				var bodyParagraphUpper = document.createTextNode("분류: "+String(itemSetofThisCategory[itemCounter].category));
				var bodyParagraphLower = document.createTextNode("ID: "+ String(itemSetofThisCategory[itemCounter].id));
				var bodyParagraphUpperTextNode = document.createElement('p');
				var bodyParagraphLowerTextNode = document.createElement('p');
				bodyParagraphUpperTextNode.appendChild(bodyParagraphUpper);
				bodyParagraphLowerTextNode.appendChild(bodyParagraphLower);
				newItemContainerDarkWrapper.appendChild(bodyParagraphUpperTextNode);
				newItemContainerDarkWrapper.appendChild(bodyParagraphLowerTextNode);
	...
	function moveFocusUpper() {
		if (currentFocusGrid.currentItem <= 0) {
            tata.error('아이템 목록의 처음입니다', 'FlexSlider', {
                position: 'br',
                duration: 800
            });
		} else {
			setTimeout(function() {
				coordArray[currentFocusGrid.currentCategory][currentFocusGrid.currentItem].style.top = '0vh';
				...
				setTimeout(function() {
					...
					setTimeout(function() {
						...
						coordArray[currentFocusGrid.currentCategory][currentFocusGrid.currentItem-1].style.top = '0vh';
	...
	function moveFocusLower() {
		if (currentFocusGrid.currentItem >= coordArray[currentFocusGrid.currentCategory].length-1) {
            tata.error('아이템 목록의 끝입니다', 'FlexSlider', {
                position: 'br',
                duration: 800
            });
		} else {
	...
	function moveFocusRightward() {
		...
		if (currentFocusGrid.currentCategory >= categoryArray.length-1) {
            tata.error('카테고리 목록의 끝입니다', 'FlexSlider', {
                position: 'br',
                duration: 800
            });
		} else {
	...
	const uriSchemeCMDtoGETformattedDataset = "http://49.50.160.79/getList.php";
	...
	xmlHTTPInstanceforFlexSlider.onload = function() {
		if (xmlHTTPInstanceforFlexSlider.status === 200) {
			if (xmlHTTPInstanceforFlexSlider.responseText !== null && xmlHTTPInstanceforFlexSlider.responseText !== undefined && JSON.parse(xmlHTTPInstanceforFlexSlider.responseText).data !== null) {
				...
				coordArray = buildFlexSlider(themessageBodyParts.data);
