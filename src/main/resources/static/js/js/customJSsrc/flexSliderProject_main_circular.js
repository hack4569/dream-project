...
window.addEventListener('load', function() {
	...
	function buildFlexSlider(givenDataSet) {
		...
		var extractedRawCategories = Array.prototype.slice.call(givenDataSet).map(function(eachElement) {
			return String(eachElement.book_title);
		...
		for (var categoriesCounter = 0; categoriesCounter < extractedRawCategories.length; categoriesCounter++) {
			...
			var itemSetofThisCategory = givenDataSet.filter(function(eachItem) {
				return eachItem.book_title.includes(extractedRawCategories[categoriesCounter]);
			});
			console.log("test", itemSetofThisCategory);
			itemSetofThisCategory.sort(function (former, latter) { return former.book_title - latter.book_title; });
			...
			for (var itemCounter = 0; itemCounter < itemSetofThisCategory.length; itemCounter++) {
				...
				var headlineText = document.createTextNode(String(itemSetofThisCategory[itemCounter].book_comment));
				...
				var bodyParagraphUpper = document.createTextNode("분류: "+String(itemSetofThisCategory[itemCounter].book_title));
				...
				newItemContainer.href = itemSetofThisCategory[itemCounter].book_url;
	...
	*/
	...
	const uriSchemeCMDtoGETformattedDataset = "http://localhost:1225/model/getList.php";
